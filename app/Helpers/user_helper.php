...
function has_subvendors() {
    ...
    $builder->where('userrole', 1)->where('block', 0)->where('id', get_user_id());
    $sub_vendors_ids = $builder->get()->getResultArray();

    if (!empty($sub_vendors_ids) && $sub_vendors_ids[0]['referred_to'] != "") {
        $sub_vendors_ids = explode(",", $sub_vendors_ids[0]['referred_to']);
        foreach ($sub_vendors_ids as $sv) {
            $builder = $db->table('users'); // Re-initialize the builder to avoid previous conditions
